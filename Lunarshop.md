...
## TL;DR
Сервис блокирует явные локальные хосты (127.0.0.1, localhost). Обход: домен с `A → публичный IP`, `AAAA → ::1`. Сервис считает хост публичным, получает редирект и конечный запрос попадает на `127.0.0.1:5001/flag`.
...
2. На PUBLIC_IP поднять redirect сервер (`poC/redirect_server.py`).

3. Подать webhook URL в сервис:  
